...
class BandtasticNew(IStrategy):
    ...
    # 在超参数优化后修改这里的Default值即可；
    buy_fastema = IntParameter(low=1, high=100, default=21, space='buy', optimize=True, load=True)
    buy_slowema = IntParameter(low=1, high=100, default=50, space='buy', optimize=True, load=True)
    ...
    sell_fastema = IntParameter(low=1, high=100, default=7, space='sell', optimize=True, load=True)
    sell_slowema = IntParameter(low=1, high=100, default=21, space='sell', optimize=True, load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 修改 EMA 计算部分
        # 使用 talib 直接计算 EMA，不使用 abstract 接口
        import talib

        # 使用正确的logger属性名
        min_period = 1
        max_period = max(self.buy_fastema.high, self.buy_slowema.high,
                        self.sell_fastema.high, self.sell_slowema.high)
        if len(dataframe) < max_period:
            self.logger.warning(f"数据长度不足 ({len(dataframe)}) 无法计算EMA")
            return dataframe

        # 仅计算有效周期
        valid_periods = [p for p in range(min_period, max_period+1) if 0 < p <= len(dataframe)]
        for period in valid_periods:
            try:
                dataframe[f'EMA_{period}'] = talib.EMA(dataframe['close'].values, timeperiod=period)
            except Exception as e:
                self.logger.warning(f"EMA计算失败 周期{period}: {str(e)}")
                dataframe[f'EMA_{period}'] = np.nan
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_rsi_enabled.value:
            conditions.append(dataframe['rsi'] < self.buy_rsi.value)  #超卖
        if self.buy_mfi_enabled.value:
            conditions.append(dataframe['mfi'] < self.buy_mfi.value)  #超卖
        ...
        # TRIGGERS  四档不同标准差的布林带，按close价格与布林带下轨的关系判断
